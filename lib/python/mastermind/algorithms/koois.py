...
def main() -> None:
    ...
    for _ in game_simulation:
        ...
        partition_counts = {
            category: get_partition_count(combinations, category) for category in categories
        }
        # Find the largest category.
        largest_category = max(partition_counts, key=partition_counts.get)
        guess = next(combination for combination in combinations if get_category(combination) == largest_category)
...
def get_category(combination: game.Code) -> game.Code:
    ...
    combination_counts: typing.List[int] = sorted(collections.Counter(combination).values(), reverse=True)
...
def get_partition_count(possible_combinations: init.Json, combination: game.Code) -> int:
    ...
    return len(collections.Counter([
        game.compare_codes(combination, possible_combination) for possible_combination in possible_combinations
    ]))


def get_all_categories(possible_combinations: init.Json) -> typing.Set[typing.Tuple[int]]:
    ...
    """
    return {tuple(get_category(possible_combination)) for possible_combination in possible_combinations}
